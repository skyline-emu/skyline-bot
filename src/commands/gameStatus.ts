...
export class GameStatus extends Command {
    ...
    async run(message: Message, content: string[]): Promise<void> {
        ...
        if (content.length == 0) {
            throw new CommandError("You must provide a game to lookup!");
        }
        ...
        let embed = new MessageEmbed()
            ...
            .setDescription("Please select your desired game from the following list:");
        ...
        collector.on('collect', async i => {
            ...
            if (i.user.id === message.author.id) {
                botMessage.edit({ components: [rowDisabled]});
                let labelNames = "";
